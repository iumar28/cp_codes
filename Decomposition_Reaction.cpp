...
void solve()
{
    int n,m;
    cin>>n>>m;
    vector<int> initial_amount(n+1,0);
    for(int i=1;i<=n;i++){cin>>initial_amount[i];}
    for(int i=1;i<=(2*m);i++){
        //first line
        int c,x;cin>>c>>x;
        int bana=0;
        ...
        for(int i=0;i<x;i++){
            ...
            int toot=first*initial_amount[c];
            initial_amount[second]+=toot;
            initial_amount[second]%=mod;;
        }
        initial_amount[c]=0;
    }
    for(int i=1;i<initial_amount.size();i++){cout<<initial_amount[i]<<endl;}
