...
void solve()
{
    int n,q;
    cin>>n>>q;
    vector<int> a(n+1),prefix(n);
    cin>>a[1];prefix[1]=a[1];
    for(int i=2;i<=n;i++){cin>>a[i];prefix[i]=prefix[i-1]+a[i];}
    while(q--){
        int first,last;cin>>first>>last;
        // cout<<prefix[first]<<" "<<prefix[last]<<endl;
        // int maxi=max(first-1,0LL);
        cout<<prefix[last]-prefix[first-1]<<endl;
    }
